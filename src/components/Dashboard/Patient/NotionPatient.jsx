...
const NotionPatient = () => {
    ...
    const fetchMedicalRecord = async () => {
      try {
        ...
        const response = await fetch(
          `https://mental-care-server-nodenet.onrender.com/api/medical-records/${profileId}`,
          {
  ...
  if (!medicalRecord) {
    return (
      <div className="min-h-[250px] bg-white flex items-center justify-center rounded-lg shadow-md">
        <div className="flex items-center space-x-2">
      ...
      <div className="space-y-4">
        ...
        {medicalRecord.MedicalRecordSpecificMentalDisorder?.length > 0 ? (
          <ul className="list-none pl-7 space-y-1">
            {medicalRecord.MedicalRecordSpecificMentalDisorder.map(
              (disorder, index) => (
                <li
                  key={index}
                ...
                >
                  ...
                  <span>
                    {disorder.MentalDisorders?.Name ||
                      `Unknown Disorder ${index + 1}`}{" "}
                    -{" "}
                    {disorder.MentalDisorders?.Description ||
          ...
          <div className="pl-7">
            <div className="flex items-center space-x-2 text-yellow-600 bg-yellow-100 p-2 rounded-lg w-fit">
              ...
              <span className="text-sm font-medium">
                No mental disorders recorded yet
              </span>
            </div>
          </div>
        )}
