...
import { BarChart, Bar, PieChart, Pie, Cell, XAxis, YAxis, Tooltip, ResponsiveContainer, RadarChart, Radar, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Legend } from "recharts";
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                try {
                    const res = await fetch("http://localhost:3000/api/patient-statistics");
                ...
                // Fetch doctors data
                try {
                    const res = await fetch("http://localhost:3000/api/doctor-profiles");
                    const data = await res.json();
                ...
                // Fetch products data
                try {
                    const res = await fetch(`https://anhtn.id.vn/subscription-service/service-packages/total?startDate=${dates.start}&endDate=${dates.end}`);
                    const data = await res.json();
                    newState.productsSold = {
                        total: data.reduce((sum, item) => sum + item.totalSubscriptions, 0).toLocaleString(),
                        details: data.map(item => ({ name: item.name, totalSubscriptions: item.totalSubscriptions.toLocaleString() })),
                    };
                } catch (err) {
                ...
                try {
                    ...
                    const subscriptionsData = await Promise.all(
                        statuses.map(async status => {
                            try {
                                const res = await fetch(`https://anhtn.id.vn/subscription-service/user-subscriptions/total?startDate=${dates.start}&endDate=${dates.end}&status=${status}`);
                                const data = await res.json();
                                return [status, data.totalCount.toLocaleString()];
                            } catch (err) {
                                console.error(`Error fetching ${status} subscriptions:`, err.message);
                                return [status, "N/A"];
                            }
                        })
                    );
                ...
                try {
                    const res = await fetch(`https://anhtn.id.vn/scheduling-service/bookings/top-doctors?StartDate=${dates.start}&EndDate=${dates.end}`);
                    const data = await res.json();
                    newState.topDoctors = {
                        total: data.topDoctors.reduce((sum, doctor) => sum + doctor.totalBookings, 0).toLocaleString(),
                ...
                try {
                    ...
                    newState.totalRevenue = `${dailySalesData.reduce((sum, item) => sum + (item.revenue || 0), 0).toLocaleString('vi-VN')} ₫`;
                } catch (err) {
    ...
    return (
        <div className="min-h-screen py-6 px-4 bg-gradient-to-br from-indigo-100 to-gray-50">
            <div className="max-w-7xl mx-auto">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                {Array.from({ length: 10 }, (_, i) => {
                                    ...
                                    return <option key={year} value={year} className="text-gray-800">{year}</option>;
                                })}
                            </select>
                        </div>
                ...
                <OverviewSection state={state} getGenderTotalUsers={getGenderTotalUsers} />
